...
export class Mat4f extends Float32Array {
    ...
    mulVec4(a: Vec4): Vec4 {
        const x = this[0] * a.x + this[4] * a.y + this[8] * a.z + this[12] * a.w;
        const y = this[1] * a.x + this[5] * a.y + this[9] * a.z + this[13] * a.w;
        const z = this[2] * a.x + this[6] * a.y + this[10] * a.z + this[14] * a.w;
        const w = this[3] * a.x + this[7] * a.y + this[11] * a.z + this[15] * a.w;
        return new Vec4(x, y, z, w);
    }

    mulVec3Proj(a: Vec3): Vec3 {
        const v4 = this.mulVec4(new Vec4(a.x, a.y, a.z, 1.0));
        const wInv = 1.0 / v4.w;
        return new Vec3(v4.x * wInv, v4.y * wInv, v4.z * wInv);
    }

    mulVec3ProjVec(a: Vec3): Vec3 {
        const v4 = this.mulVec4(new Vec4(a.x, a.y, a.z, 0.0));
    ...
    mulVec3Affine_(a: Vec3, o: Vec3) {
        const x = this[0] * a.x + this[4] * a.y + this[8] * a.z + this[12];
        const y = this[1] * a.x + this[5] * a.y + this[9] * a.z + this[13];
        const z = this[2] * a.x + this[6] * a.y + this[10] * a.z + this[14];
        o.x = x;
        o.y = y;
        o.z = z;
    }
    ...
    mulVec3AffineVec_(a: Vec3, o: Vec3) {
        const x = this[0] * a.x + this[4] * a.y + this[8] * a.z;
        const y = this[1] * a.x + this[5] * a.y + this[9] * a.z;
        const z = this[2] * a.x + this[6] * a.y + this[10] * a.z;
        o.x = x;
        o.y = y;
        o.z = z;
    }
    ...
    static fromRowMajor(a: ArrayLike<number> | number[][]) {
        ...
        const flatArr = a as ArrayLike<number>;
    ...
    static fromLookAt(eye: Vec3, center: Vec3, up: Vec3) {
        ...
        let u = up.normalize();
        const r = Vec3.cross(u, f).normalize();
        u = Vec3.cross(f, r);
    ...
    static fromPersp(fovDeg: number, aspect: number, near: number, far: number) {
        const h = near * Math.tan(fovDeg / 2 * Math.PI / 180) * 2;
        const w = h * aspect;
    ...
    decomposeToTRS(): [Vec3, Vec4, Vec3] {
        ...
        if (tr > 0.0) {
            ...
            const r = Math.sqrt(1.0 + this[0] - this[5] - this[10]);
            const s = 0.5 / r;
            R = new Vec4(
                0.5 * r,
                (this[1] + this[4]) * s,
            ...
            const r = Math.sqrt(1.0 + this[5] - this[0] - this[10]);
            const s = 0.5 / r;
            R = new Vec4(
                (this[4] + this[1]) * s,
                0.5 * r,
            ...
            const r = Math.sqrt(1.0 + this[10] - this[0] - this[5]);
            const s = 0.5 / r;
            R = new Vec4(
                (this[8] + this[2]) * s,
                (this[9] + this[6]) * s,
    ...
    invertTRS(): Mat4f {
        ...
        const u = Vec3.fromArray(this, 0);
        const v = Vec3.fromArray(this, 4);
        const w = Vec3.fromArray(this, 8);
        const t = Vec3.fromArray(this, 12);
...
export class Mat3f extends Float32Array {
    ...
    mulVec3(a: Vec3): Vec3 {
        const x = this[0] * a.x + this[3] * a.y + this[6] * a.z;
        const y = this[1] * a.x + this[4] * a.y + this[7] * a.z;
        const z = this[2] * a.x + this[5] * a.y + this[8] * a.z;
...
function test() {
    ...
    const persp = Mat4f.fromPersp(30, 1.0, 1, 100);
    ...
    for (const v of vals) {
        const near = 1;
        const far = 100;
        const proj = persp.mulVec3Proj(v);
        const unprojZ = near * far / ((near - far) * proj.z + far);
        console.log('proj', v, '=>', proj.z, 'and back? =>', unprojZ);
    }

    const lookAt = Mat4f.fromLookAt(new Vec3(0, 0, 4), new Vec3(0, 0, 0), new Vec3(0, 1, 0));
    console.log(`lookAt: ${lookAt}`);

    for (let i = 0; i < 1; i++) {
        const input = new Vec4(Math.random(), Math.random(), Math.random(), Math.random()).normalize();
        const m = Mat4f.fromQuat(input);
        const [T, R, S] = m.decomposeToTRS();
