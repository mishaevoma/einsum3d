...
import { generateEinsumLayout, generateTensorGeometry } from '@/src/llm/EinsumLayout';
...
describe('generateEinsumLayout', () => {
  it('lays out operands and the derived result', () => {
    ...
    expect(layout.cubes.every((cube, index) => cube.idx === index)).toBe(true);
  });
  ...
  it('still produces cubes for an invalid equation', () => {
    ...
    expect(layout.cubes.every((cube) => cube.kind === 'operand')).toBe(true);
  });
});
